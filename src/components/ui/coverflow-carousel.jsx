    ...
    >
      ...
      {showCaption && active?.title && (
        ...
        >
          ...
          {active.subtitle && (
            <p className="mt-2 text-sm font-bold text-dark/75 text-center max-w-sm">
              {active.subtitle}
            </p>
          )}
          {active.meta && active.meta.length > 0 && (
            <dl className="mt-6 w-full max-w-[280px] text-sm">
              {active.meta.map((row) => (
                <div key={row.label} className="flex justify-between py-2 border-b border-primary/10 last:border-0">
                  <dt className="font-heading font-bold text-xs uppercase tracking-widest text-primary/70">{row.label}</dt>
                  <dd className="font-heading font-black text-dark">{row.value}</dd>
                </div>
              ))}
            </dl>
          )}
